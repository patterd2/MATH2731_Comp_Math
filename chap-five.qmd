...
$$
u^{(n)} = f\big(t, u, u', u'',\dotsc, u^{(n-1)}\big),
$$
where $u^{(n)} := d^nu(t)/du^n$. Now introduce the auxiliary variables:
$$
y_0 = u, \quad y_1 = u', \quad \ldots, \quad y_{n-1} = u^{(n-1)}
$$
